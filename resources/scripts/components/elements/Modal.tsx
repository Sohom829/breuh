...
const ModalContainer = styled.div<{ alignTop?: boolean }>`    
    ...
    & > .close-icon {
        ${tw`absolute right-0 p-2 text-black cursor-pointer opacity-50 transition-all duration-150 ease-linear hover:opacity-100`};
        top: -2.5rem;
        ...
        <Fade in={render} timeout={150} appear={appear || true} unmountOnExit onExited={() => onDismissed()}>
            ...
            >
                <ModalContainer alignTop={top}>
                    ...
                    <div
                        css={tw`bg-white p-3 sm:p-4 md:p-6 rounded-2xl shadow-md transition-all duration-150`}
                    >
                        {children}
                    </div>
                </ModalContainer>
