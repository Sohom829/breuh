...
export default ({ subuser }: Props) => {
    ...
    return (
        <GreyRowBox $hoverable={false} css={tw`mb-2`}>
            ...
            <div css={tw`w-10 h-10 rounded-full bg-white border-2 p-1 border-dactyl-white overflow-hidden hidden md:block`}>
                <img css={tw`w-full h-full`} src={`${subuser.image}?s=400`} />
            </div>
            <div css={tw`ml-4 flex-1 overflow-hidden`}>
                <p css={tw`text-sm text-dactyl-purple2 truncate`}>{subuser.email}</p>
            </div>
            <div css={tw`ml-4`}>
            ...
            <div css={tw`ml-4 hidden md:block`}>
                <p css={tw`font-medium text-center text-dactyl-orange2`}>
                ...
                <>
                    <Can action={'user.update'}>
                        <button
                            ...
                            css={tw`block text-sm p-2 text-dactyl-purple2 rounded-full mx-4`}
                            onClick={() => setVisible(true)}
                        >
                            <FontAwesomeIcon icon={faPencilAlt} style={{ width: '13px', height: '13px' }} />
                        </button>
                    </Can>
